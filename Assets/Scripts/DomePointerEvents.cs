...
public class DomePointerEvents : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
    ...
    public List<GameObject> commentManagerList;
    ...
    private void Awake()
    {
        ...
        testText.text = "COMMENT-MODE (microphone)";
        //CreateDemoComments();
    }
    ...
    private void Start()
    {
        ...
        path = pdp + "/" + videoManager.videoPlayer.clip.name ;
    ...
    {
        if(inputManager.currentCommentState == InputManager.CommentState.Microphone) 
        { 
            videoManager.PauseToggle();
            microphoneRecordingIcon.SetActive(true);
    ...
    public void StopTimerandOpenKeyboard(bool fromThisClass)
    {
        ...
        if (inputManager.currentCommentState == InputManager.CommentState.Microphone)
        {
            microphoneRecordingIcon.SetActive(false);
            inputManager.StopRecording();
        }
        else if (inputManager.currentCommentState == InputManager.CommentState.Text)
        {
            videoManager.PauseToggle();
    ...
    {
        testText.text = "VIEW-MODE";
        videoManager.ResetVideo();

        foreach (GameObject commentManager in commentManagerList)
        {
            commentManager.GetComponent<CommentManagerObject>().LoadCommentsforCurrentVideo(path);
    ...
    public void PrepCommentMode()
    {
        ...
        {
            testText.text = "COMMENT-MODE (text)";
        }
           
        videoManager.ResetVideo();
        foreach (GameObject commentManager in commentManagerList)
        {
            commentManager.GetComponent<CommentManagerObject>().HideAllComments2();
        }
    }

    public void PrepAllCommentsMode()
    {
        testText.text = "ALL-COMMENTS-MODE";
        videoManager.ResetVideo();
        foreach (GameObject commentManager in commentManagerList)
        {
            commentManager.GetComponent<CommentManagerObject>().HideAllComments3();
            commentManager.GetComponent<CommentManagerObject>().LoadCommentsforCurrentVideo(path);
    ...
    public void CheckKeyboardStatusAndRetriveInput()
    {
        ...
        { 
            ...
            {
                CommentManagerObject newCommentManager = Instantiate(commentManagerObject, new Vector3(1, 1, 1), Quaternion.identity);            
                var newComment = Instantiate(commentInstancePrefab);
    ...
    public void PauseCommentsVideoManagers()
    {
        foreach (GameObject commentManager in commentManagerList)
        {
            commentManager.GetComponent<CommentManagerObject>().PauseorUnpauseCommentsCorroutines();
        }
    }
    ...
    public void CreateDemoComments()
    {
        ...
        PrepCommentMode();
        newCommentManager1.container.SetActive(false);
        newCommentManager2.container.SetActive(false);
        newCommentManager3.container.SetActive(false);
        newCommentManager4.container.SetActive(false);
    }
